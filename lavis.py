...
def get_pydot_graph(layers, output_shape=True, rankdir="LR"):
    ...
    """
    pydot_graph = pydot.Dot('Network', graph_type='digraph', rankdir=rankdir, bgcolor='transparent')
    pydot_nodes = {}
    pydot_edges = []
    for i, layer in enumerate(layers):
        ...
        if hasattr(layer, 'nonlinearity'):
            ...
            if rankdir != "LR":
                label += "</TR><TR>"
            label += '<TD BGCOLOR="#FFFF00">' + '{0}'.format(nonlinearity)+"</TD></TR></TABLE>>"            
            color="none"
...
def draw_to_notebook(layers, **kwargs):
    ...
    :parameters:
        ...
        - **kwargs: see docstring of get_pydot_graph for other options
    """
    from IPython.display import SVG  # needed to render in notebook
